...
export class Player extends AnimatedGameObject implements Health {
  ...
  update(x: number, y: number, degrees: number = 0, animation: AnimationKey = "idle"): void {
    ...
    this.tryMove(x, 0);
    this.tryMove(0, y);
    super.update(0, 0, degrees, animation);
  }
  ...
  private tryMove(dx: number, dy: number): void {
    ...
    const hitsBefore = this.solidHits();
    this._x += dx;
    this._y += dy;

    const blocked = this.solidHits().some((obj) => !hitsBefore.includes(obj));
    if (blocked) {
      this._x -= dx;
      this._y -= dy;
    }
  ...
  private solidHits() {
    return this.collider.collidesAny(game.gameObjects).filter((obj) => obj.solid);
  }
